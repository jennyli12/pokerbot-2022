...
RAISES = [0.75, 2]  # percentage of pot for raise
RAISE_LETTER = ['x', 'y']
RAISE_LETTER_INV = {'x': 0, 'y': 1}
RAISE_SET = {'x', 'y', 'a'}
...
class RoundState(namedtuple('_RoundState', ['button', 'street', 'pips', 'stacks', 'hands', 'deck', 'history'])):
    ...
    def get_actions(self):
        ...
        raise_actions = []
        too_many_raises = len(self.history) >= 3 and self.history[-1] in RAISE_SET and self.history[-2] in RAISE_SET and \
            self.history[-3] in RAISE_SET
